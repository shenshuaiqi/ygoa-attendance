...
export const hasChild = (item) => {
  return item.children && item.children !== null && item.children.length !== 0
}
...
export const compareRoute = (url, list, whiteArr) => {
  for (let i = 0; i < list.length; i++) {
    if (url === list[i].name || url === homeName) {
      return true;
    }
  }
  return false;
...
export function downloadFile(url, callback) {
  callback(url).then(response => {
    if (response.status == 200 && response.data.code == '0000') {
      let data = response.data.result;
      let webPath = data.webPath;
      let address = data.address;
      let fileName = data.fileName;
      let path = webPath + '/' + address;
      if (!path) {
        return;
      }
