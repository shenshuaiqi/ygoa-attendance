<template>
  <Modal class="user-box" :value="dialog" draggable scrollable :closable="false" title="选择人员" width="800">
    <div slot="footer">
      ...
      <Row :gutter="16">
        ...
        <i-col span="10">
          ...
          <List v-if="!spinShow" size="small" style="height: 300px;overflow: auto">
                <ListItem @click.native="chose(item)" v-for="(item,index) in choseData" :key="index" style="display: block">
...
<script>
  ...
  export default {
    ...
    methods: {
      chose(obj) {
        this.$emit('diachose', {data: JSON.stringify(obj), type: this.type})
